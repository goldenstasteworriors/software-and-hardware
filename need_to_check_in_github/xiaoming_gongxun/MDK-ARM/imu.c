...
/*
void mahony_ahrs_updateIMU(void)
{
  float recipNorm;
...
/*	imu_get_total_angle:
	获取总的旋转角度 total_yaw	// 注意！使用前要将yaw0赋值为上电时的yaw
*/
void imu_get_total_angle(){
	//获取数据
		mpu_get_data();
		imu_ahrs_update();
		imu_attitude_update();
	//临界处理
		if(imu.yaw-imu_last_yaw>180){
			cnt=cnt-1;
		}
		else if(imu.yaw-imu_last_yaw<-180){
			cnt=cnt+1;
		}
//		printf("%f\r\n",fabs(abs(cnt)*360+imu.yaw-yaw0));
		if(fabs(abs(cnt)*360+imu.yaw-yaw0)>0.20)
			yaw0=imu.yaw;
		imu_total_yaw=abs(cnt)*360+imu.yaw-yaw0;		
		imu_last_yaw=imu.yaw;	
}
