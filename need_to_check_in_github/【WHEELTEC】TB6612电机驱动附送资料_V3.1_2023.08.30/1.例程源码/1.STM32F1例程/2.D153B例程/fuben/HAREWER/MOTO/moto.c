...
void moto2(int mode)
{
	if(mode==1)    //反转
	{
	 GPIO_SetBits(GPIOB, GPIO_Pin_3);	 // 高电平      PB3 --- AIN2      1   
	 GPIO_ResetBits(GPIOB, GPIO_Pin_4);	 // 低电平}   PB4 --- AIN1      0
	
	 GPIO_SetBits(GPIOB, GPIO_Pin_5);     //高电平   PB5 --- BIN2       1
	 GPIO_ResetBits(GPIOA, GPIO_Pin_15);  // 低电平   PA15 --- BIN1       0
	
	}
	 if(mode==0)   //正传
	{
	 GPIO_SetBits(GPIOB, GPIO_Pin_4);	 // 高电平       PB4 --- AIN1     1
	 GPIO_ResetBits(GPIOB, GPIO_Pin_3);	 // 低电平}    PB3 --- AIN2     0
	
	 GPIO_ResetBits(GPIOA, GPIO_Pin_15);     //高电平   PB13 --- BIN2     0
	 GPIO_SetBits(GPIOB, GPIO_Pin_5);  // 低电平   PB12 --- BIN1         1
	 }
